...
class CloudletResource(ModelResource):
    ...
    def get_search(self, request, **kwargs):
        ...
        for cloudlet in Cloudlet.objects.all():
            ...
            geo_distance = ip_location.geo_distance(
                latitude, longitude,
                float(cloudlet.latitude),
                float(cloudlet.longitude)
            )
            cloudlet.cost = geo_distance
            cloudlet_list.append(cloudlet)

        top_cloudlets = heapq.nsmallest(
            SEARCH_COUNT, cloudlet_list, key=itemgetter('cost'))
